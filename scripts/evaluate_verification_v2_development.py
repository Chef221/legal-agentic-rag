...
- Structured dimension diagnostics computed and reported per dimension and human error tag
...
class V2DevelopmentBenchmarkEvaluator:
    ...
    def run(self) -> dict[str, Any]:
        ...
        if self._preflight_only:
            ...
            v0_binary_metrics = self._compute_binary_metrics(all_claim_targets, v0_claim_preds, pred_key="v0_binary_prediction")
            v0_answer_metrics = self._compute_answer_level_metrics(arm_targets, v0_arm_results)
            v1_binary_metrics = self._compute_binary_metrics(all_claim_targets, v1_claim_preds, pred_key="v1_binary_prediction")
            v1_three_way = self._compute_three_way_metrics(all_claim_targets, v1_claim_preds, label_key="v1_three_way_prediction")
        ...
        for pass_idx in range(1, self._repeat_count + 1):
            pass_claim_preds, pass_arm_res, pass_struct_res, err_count, pass_traces = self._execute_v2_pass(
        ...
        # 12. Compute Dimension-Level Diagnostics
    ...
    def _validate_sources(self) -> dict[str, Any]:
        ...
        sources = [
            (self._forensic_packets_path, CANONICAL_FORENSIC_REVIEW_ZIP_SHA256, ".zip", "forensic review packets"),
            (self._forensic_labels_path, CANONICAL_FORENSIC_LABELS_SHA256, ".json", "forensic labels"),
            (self._control_packets_path, CANONICAL_CONTROL_REVIEW_ZIP_SHA256, ".zip", "control review packets"),
            (self._control_labels_path, CANONICAL_CONTROL_LABELS_SHA256, ".json", "control labels"),
            (self._v1_evidence_path, CANONICAL_V1_EVIDENCE_ZIP_SHA256, ".zip", "canonical V1 baseline evidence archive"),
        ]
    ...
    def _construct_arm_target(
        ...
        *,
        ...
        raw_resp = arm_packet_data.get("historical_response", {}) or arm_packet_data.get("response", {})
        resp_obj = AnswerResponse.model_validate(raw_resp)

        raw_ev = arm_packet_data.get("selected_evidence", []) or arm_packet_data.get("reconstructed_evidence", [])
    ...
    def _load_canonical_v1_predictions(
        ...
        for target in claim_targets:
            ...
            # Add is_correct flag for convenience
            is_correct = (
    ...
    def _validate_canonical_config(self) -> None:
        """Assert that runtime configuration strictly matches canonical V2 parameters."""
    ...
    def _validate_runtime_provider_identity(self, provider: ChatModelProvider) -> None:
        ...
        if self._custom_provider is not None:
            return
        if provider.provider_name != CANONICAL_V2_BACKEND:
            raise DataValidationError(
                f"INVALID_VERIFIER_BENCHMARK_PROVENANCE: Provider name mismatch: expected '{CANONICAL_V2_BACKEND}', got '{provider.provider_name}'"
            )
        if provider.model_name != CANONICAL_V2_MODEL_NAME:
            raise DataValidationError(
                f"INVALID_VERIFIER_BENCHMARK_PROVENANCE: Model name mismatch: expected '{CANONICAL_V2_MODEL_NAME}', got '{provider.model_name}'"
            )
        if provider.model_revision != CANONICAL_V2_MODEL_REVISION:
            raise DataValidationError(
                f"INVALID_VERIFIER_BENCHMARK_PROVENANCE: Model revision mismatch: expected '{CANONICAL_V2_MODEL_REVISION}', got '{provider.model_revision}'"
            )
    ...
    def _compute_binary_metrics(
        ...
        supported_retention = tp / (tp + fn) if (tp + fn) else 0.0  # Recall on positives
        negative_catch = tn / (tn + fp) if (tn + fp) else 0.0      # Specificity / Recall on negatives
        f1 = 2 * precision * supported_retention / (precision + supported_retention) if (precision + supported_retention) else 0.0
    ...
    def _compute_paired_metrics(
        ...
        """Compute paired correctness deltas between V1 and V2."""
    ...
    def _compute_answer_level_metrics(
        ...
        """Compute answer-level validity metrics."""
        valid_retained = 0
        valid_total = 0
        invalid_caught = 0
        invalid_total = 0
    ...
    def _compute_dimension_diagnostics(
        ...
        """Compute aggregate dimension activations and correlation with human error tags."""
        dimensions = [
            "actor_role",
            "action_object",
            "condition_exception",
    ...
    def _compute_all_metrics(
        ...
        return {
            "v0_claim_binary": self._compute_binary_metrics(claim_targets, v0_claim_preds, pred_key="v0_binary_prediction"),
            "v1_claim_binary": self._compute_binary_metrics(claim_targets, v1_claim_preds, pred_key="v1_binary_prediction"),
            "v2_claim_binary": self._compute_binary_metrics(claim_targets, v2_claim_preds, pred_key="v2_binary_prediction"),
            "v1_three_way": self._compute_three_way_metrics(claim_targets, v1_claim_preds, label_key="v1_three_way_prediction"),
            "v2_three_way": self._compute_three_way_metrics(claim_targets, v2_claim_preds, label_key="v2_three_way_prediction"),
            "paired_v1_vs_v2": self._compute_paired_metrics(claim_targets, v1_claim_preds, v2_claim_preds),
            "v0_answer_metrics": self._compute_answer_level_metrics(arm_targets, v0_arm_results),
            "v2_answer_metrics": self._compute_answer_level_metrics(arm_targets, v2_arm_results),
        }
    ...
    def _collect_runtime_identity(
        ...
        """Collect execution environment and provider provenance."""
    ...
    def _build_execution_identity(
        ...
        """Build execution identity metadata block."""
        return {
            "schema_version": "1.0",
    ...
    def _build_reports(
        ...
        """Package detailed and decision reports."""
        comparisons: list[dict[str, Any]] = []
        for t, p0, p1, p2 in zip(all_claim_targets, v0_claim_preds, v1_claim_preds, v2_pass1_preds, strict=True):
            comparisons.append(
                {
        ...
        # Determine development candidate decision
        v2_bin = metrics_report["v2_claim_binary"]
        v1_bin = metrics_report["v1_claim_binary"]
        paired = metrics_report["paired_v1_vs_v2"]

        is_promising = (
            v2_bin["accuracy"] > v1_bin["accuracy"]
            and v2_bin["negative_catch"] > v1_bin["negative_catch"]
            and v2_bin["supported_retention"] >= v1_bin["supported_retention"]
    ...
    def _write_outputs(
        ...
        if v2_pass1_preds:
            _write_jsonl(self._output_dir / "results" / "v2_claim_predictions_pass1.jsonl", v2_pass1_preds)
        if v2_pass2_preds:
            _write_jsonl(self._output_dir / "results" / "v2_claim_predictions_pass2.jsonl", v2_pass2_preds)
        if comparisons:
        ...
        if self._package_zip_path:
            ...
            with zipfile.ZipFile(self._package_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V2 Structured Semantic Verifier Development Benchmark Harness")
    parser.add_argument("--forensic-packets", type=Path, required=True, help="Path to verification-forensic-review-packets.zip")
    parser.add_argument("--forensic-labels", type=Path, required=True, help="Path to verification-human-forensic-labels-v1.json")
    parser.add_argument("--control-packets", type=Path, required=True, help="Path to verification-positive-control-review-packets-v1.zip")
    parser.add_argument("--control-labels", type=Path, required=True, help="Path to verification-positive-control-human-labels-v1.json")
    parser.add_argument("--v1-evidence", type=Path, required=True, help="Path to verification-semantic-benchmark-evidence.zip")
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory to write evaluation outputs")
    parser.add_argument("--package-zip", type=Path, default=None, help="Optional path to package output ZIP")
    parser.add_argument("--preflight-only", action="store_true", help="Validate sources and compute baselines without running real Qwen")
    parser.add_argument("--candidate-id", type=str, default="V2-D1", help="Candidate identifier (default V2-D1)")
    parser.add_argument("--device", type=str, default=CANONICAL_V2_DEVICE, help="Inference device (default cuda)")
    parser.add_argument("--repeat-count", type=int, default=CANONICAL_REPEAT_COUNT, help="Repeat count for stability (default 2)")
    parser.add_argument("--max-structured-retries", type=int, default=CANONICAL_V2_MAX_STRUCTURED_RETRIES, help="Max structured retries (default 1)")
    ...
    try:
        ...
        print(json.dumps({"verdict": "V2_DEVELOPMENT_BENCHMARK_ERROR", "error": str(exc)}, indent=2), file=sys.stderr)
        return 1

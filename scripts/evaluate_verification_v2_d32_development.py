#!/usr/bin/env python3
"""V2-D3.2 Frozen D3 Base + Strict Contradiction Confirmation Development Benchmark Harness.

This script executes the controlled offline development benchmark for candidate V2-D3.2
...
- Primary Comparator D3 predictions loaded from: verification-v2-d3-development-evidence.zip
- Diagnostic Comparator D3.1 predictions loaded from: verification-v2-d31-development-evidence.zip

Key features in V2-D3.2:
- Two-Stage Asymmetric Architecture (2 calls per claim):
  * Call A: Frozen D3 Base Verifier (ENTAILS, CONTRADICTS, DOES_NOT_ESTABLISH)
  * Call B: Strict Contradiction Confirmation Filter (same_material_proposition, cannot_both_be_true)
- Trusted Override Rule:
  * If conflict check confirms (same_material_proposition=True and cannot_both_be_true=True):
      final_label = CONTRADICTED (Override applied)
  * Else:
      final_label = base_d3_label (D3 base label strictly preserved)
- Call Accounting: 38 claims * 2 calls * 2 passes = 152 base provider calls.
- Pre-Registered Selection Gate: D32_SUPERSEDES_D3 vs KEEP_D3 based on exact integer thresholds.
"""
...
import legal_agentic_rag.generation.structured_semantic_verifier_d32
...
import legal_agentic_rag.generation.structured_semantic_verifier_d32_conflict
from legal_agentic_rag.generation.structured_semantic_verifier_d32_conflict import (
    STRUCTURED_SEMANTIC_D32_CONFLICT_SYSTEM_INSTRUCTION,
...
# Canonical Frozen Benchmark Checksums (7 Canonical Sources)
...
class V2D32DevelopmentBenchmarkEvaluator:
    """Evaluates candidate V2-D3.2 against the composite 38-claim development benchmark."""
    ...
    def __init__(
        ...
        *,
        ...
        self._d31_evidence_path = d31_evidence_path
    ...
    def evaluate(self) -> dict[str, Any]:
        ...
        _LOGGER.info("Starting V2-D3.2 Development Benchmark Evaluation...")

        # 1. Verify inputs and canonical hashes (7 inputs)
        ...
        # 4. Load canonical V1, D3, and D3.1 predictions from evidence archives
        v1_claim_preds = self._load_v1_baseline_predictions()
        d3_claim_preds = self._load_d3_comparison_predictions()
        d31_claim_preds = self._load_d31_diagnostic_predictions()
        ...
        if self._preflight_only:
            ...
            preflight_report = self._build_preflight_report(
                ...
                d31_claim_preds=d31_claim_preds,
            )
        ...
        # 7. Initialize V2-D3.2 verifier provider & validate runtime identity
        ...
        verifier = StructuredSemanticCitationVerifierD32(
        ...
        _LOGGER.info("Executing V2-D3.2 Development Pass 1 (Benchmark Evaluation)...")
        ...
        _LOGGER.info("Executing V2-D3.2 Development Pass 2 (Two-Pass Stability)...")
        ...
        # 11. Compute Metrics (Pass 1 authoritative against D3 comparator)
        all_metrics = self._compute_all_metrics(
            ...
            d3_claim_preds=d3_claim_preds,
            d31_claim_preds=d31_claim_preds,
            d32_claim_preds=pass1_claim_preds,
            v0_arm_results=v0_arm_results,
            d32_arm_results=pass1_arm_results,
        )

        # 12. Compute Strict Conflict Precision & Recall Diagnostics
        strict_conflict_diag = self._compute_strict_conflict_diagnostics(
            targets=claim_targets,
            d32_preds=pass1_claim_preds,
        )

        # 13. Compute D3.1 Learning Diagnostic
        d31_learning_diag = self._compute_d31_learning_diagnostic(
            targets=claim_targets,
            d31_preds=d31_claim_preds,
            d32_preds=pass1_claim_preds,
        )

        # 14. Compute Gain Preservation & Forensic Error Group Diagnostics
        ...
        # 15. Build Final Evaluation and Decision Reports
        ...
        final_report, decision_report = self._build_reports(
            ...
            strict_conflict_diag=strict_conflict_diag,
            d31_learning_diag=d31_learning_diag,
        ...
        _LOGGER.info("V2-D3.2 Development Benchmark complete. Verdict: %s", final_report["verdict"])
        return final_report

    def _verify_canonical_source_checksums(self) -> dict[str, dict[str, Any]]:
        """Verify the 7 canonical input datasets against frozen SHA-256 signatures."""
        sources = {
            ...
            "canonical_D31_diagnostic_evidence_archive": (
                self._d31_evidence_path,
                CANONICAL_D31_EVIDENCE_ZIP_SHA256,
            ),
        }
    ...
    def _build_execution_identity(
        ...
        return {
            "schema_version": "1.0",
            "artifact_type": "v2_d32_development_execution_identity",
            ...
            "implementation_identities": {
                ...
                "structured_semantic_verifier_d32_conflict_sha256": self._sha256_file(
                    Path(legal_agentic_rag.generation.structured_semantic_verifier_d32_conflict.__file__)
                ),
                "evaluate_verification_v2_d32_development_sha256": self._sha256_file(
                    Path(__file__)
                ),
            },
            "prompt_identities": {
                "d3_base_system_instruction_sha256": d3_sys_sha,
                "d32_conflict_system_instruction_sha256": d32_conflict_sys_sha,
            },
        }
    ...
    def _load_d31_diagnostic_predictions(self) -> list[dict[str, Any]]:
        with zipfile.ZipFile(self._d31_evidence_path, "r") as zf:
            with zf.open("results/v2_d31_claim_predictions_pass1.jsonl") as f:
                lines = [line.decode("utf-8").strip() for line in f if line.strip()]
        return [json.loads(line) for line in lines]
    ...
    def _validate_package_provenance(self) -> None:
        ...
        try:
            ...
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Failed to read installed distribution version: {exc}"
            ) from exc
        ...
        if (
            ...
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Package version mismatch: source='{source_ver}', installed='{dist_ver}', expected='{CANONICAL_PACKAGE_VERSION}'"
            )
    ...
    def _validate_canonical_provenance(self) -> None:
        ...
        if self._candidate_id != CANONICAL_CANDIDATE_ID:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Candidate ID mismatch: expected '{CANONICAL_CANDIDATE_ID}', got '{self._candidate_id}'"
            )
        self._validate_package_provenance()
        if not self._is_git_worktree_clean():
            raise DataValidationError(
                "INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Git worktree must be clean for canonical real execution"
            )
        git_head = self._get_git_commit()
        if len(git_head) != 40 or not all(c in "0123456789abcdef" for c in git_head.lower()):
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Invalid 40-char Git HEAD commit: '{git_head}'"
            )
        if self._device != CANONICAL_V3_DEVICE:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected device '{CANONICAL_V3_DEVICE}', got '{self._device}'"
            )
        if self._torch_dtype != CANONICAL_V3_TORCH_DTYPE:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected torch_dtype '{CANONICAL_V3_TORCH_DTYPE}', got '{self._torch_dtype}'"
            )
        if self._temperature != CANONICAL_V3_TEMPERATURE:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected temperature {CANONICAL_V3_TEMPERATURE}, got {self._temperature}"
            )
        if self._repeat_count != CANONICAL_REPEAT_COUNT:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected repeat count {CANONICAL_REPEAT_COUNT}, got {self._repeat_count}"
            )
        if self._max_structured_output_retries != CANONICAL_V3_MAX_STRUCTURED_RETRIES:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected max retries {CANONICAL_V3_MAX_STRUCTURED_RETRIES}, got {self._max_structured_output_retries}"
            )
        if self._max_input_tokens != CANONICAL_V3_MAX_INPUT_TOKENS:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected max input tokens {CANONICAL_V3_MAX_INPUT_TOKENS}, got {self._max_input_tokens}"
            )
        if self._max_output_tokens != CANONICAL_V3_MAX_OUTPUT_TOKENS:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected max output tokens {CANONICAL_V3_MAX_OUTPUT_TOKENS}, got {self._max_output_tokens}"
            )
        if self._timeout_seconds != CANONICAL_V3_TIMEOUT_SECONDS:
            raise DataValidationError(
                f"INVALID_V2_D32_DEVELOPMENT_PROVENANCE: Expected timeout {CANONICAL_V3_TIMEOUT_SECONDS}, got {self._timeout_seconds}"
            )
        if not self._is_cuda_available():
            raise DataValidationError(
                "INVALID_V2_D32_DEVELOPMENT_PROVENANCE: CUDA device is required for canonical real execution"
            )
    ...
    def _run_inference_pass(
        ...
        for arm in arm_targets:
            ...
            for claim in arm.claims:
                ...
                if is_exec_err:
                    ...
                    base_d3_val = "EXECUTION_ERROR"
                    override_applied = False
                    assessment_dict = None
                elif assess is not None:
                    bin_pred = (
                        BinaryPrediction.ACCEPT.value
                        if assess.final_label == SemanticSupportLabel.SUPPORTED
                        else BinaryPrediction.REJECT.value
                    )
                    three_way_pred = assess.final_label.value.upper()
                    base_d3_val = assess.base_d3_label.value.upper()
                    override_applied = assess.override_applied
                    assessment_dict = {
                        "claim_id": assess.claim_id,
                        "base_d3_label": base_d3_val,
                        "final_label": three_way_pred,
                        "override_applied": override_applied,
                        "d3_assessment": assess.d3_assessment.model_dump() if assess.d3_assessment else None,
                        "conflict_assessment": assess.conflict_assessment.model_dump() if assess.conflict_assessment else None,
                ...
                claim_preds.append({
                    ...
                    "v2_d32_binary_prediction": bin_pred,
                    "v2_d32_three_way_prediction": three_way_pred,
                    "base_d3_label": base_d3_val,
                    "override_applied": override_applied,
                    "telemetry": telemetry_dict,
        ...
        claim_provider_call_count_sum = sum(
            p.get("telemetry", {}).get("total_provider_calls", 0) for p in claim_preds
        )
        d3_base_calls_sum = sum(
            p.get("telemetry", {}).get("d3_base_calls", 0) for p in claim_preds
        )
        conflict_calls_sum = sum(
            p.get("telemetry", {}).get("conflict_calls", 0) for p in claim_preds
        )
        d3_retries_sum = sum(
            p.get("telemetry", {}).get("d3_base_retries", 0) for p in claim_preds
        )
        conflict_retries_sum = sum(
            p.get("telemetry", {}).get("conflict_retries", 0) for p in claim_preds
        )
        semantic_execution_error_count = sum(
            1 for p in claim_preds if p.get("v2_d32_three_way_prediction") == "EXECUTION_ERROR"
        )
        overrides_in_pass = sum(
            1 for p in claim_preds if p.get("override_applied") is True
        )
        ...
        pass_telemetry = {
            ...
            "d3_base_calls": d3_base_calls_sum,
            "d3_base_retries": d3_retries_sum,
            "conflict_calls": conflict_calls_sum,
            "conflict_retries": conflict_retries_sum,
            "total_structured_retries": d3_retries_sum + conflict_retries_sum,
            "semantic_execution_errors": semantic_execution_error_count,
            "overridden_claims_count": overrides_in_pass,
        }
    ...
    def _evaluate_stability(
        ...
        for t in claim_targets:
            ...
            l1 = p1.get("v2_d32_three_way_prediction", "EXECUTION_ERROR")
            l2 = p2.get("v2_d32_three_way_prediction", "EXECUTION_ERROR")
    ...
    def _compute_all_metrics(
        ...
        d32_claim_preds: list[dict[str, Any]],
        v0_arm_results: dict[str, CitationVerificationResult],
        d32_arm_results: dict[str, CitationVerificationResult],
    ) -> dict[str, Any]:
        ...
        d32_binary = self._compute_binary_metrics(
            claim_targets, d32_claim_preds, pred_key="v2_d32_binary_prediction"
        )
        ...
        d32_three_way = self._compute_three_way_metrics(
            claim_targets, d32_claim_preds, label_key="v2_d32_three_way_prediction"
        )
        ...
        contra_diag = self._compute_contradiction_diagnostics(
            claim_targets, d3_claim_preds, d31_claim_preds, d32_claim_preds
        )
        ...
        d32_answer = self._compute_answer_level_metrics_from_preds(
            arm_targets, d32_claim_preds, pred_key="v2_d32_three_way_prediction"
        )

        answer_deltas = {
            "d32_vs_d3_valid_retention_delta": round(
                d32_answer["valid_answer_retention_rate"] - d3_answer["valid_answer_retention_rate"], 4
            ),
            "d32_vs_d3_invalid_catch_delta": round(
                d32_answer["invalid_answer_catch_rate"] - d3_answer["invalid_answer_catch_rate"], 4
            ),
            "d32_vs_d3_answer_accuracy_delta": round(
                d32_answer["answer_level_accuracy"] - d3_answer["answer_level_accuracy"], 4
            ),
        }
        ...
        return {
            ...
            "v2_d32_claim_binary": d32_binary,
            ...
            "v2_d32_answer_metrics": d32_answer,
            "v2_d32_vs_d3_answer_deltas": answer_deltas,
        }
    ...
    def _compute_contradiction_diagnostics(
        ...
        d32_map = {
            (r["question_id"], r["arm_id"], r["claim_id"]): r.get("v2_d32_three_way_prediction")
            for r in d32_preds
        }
        ...
        total_gold = len(gold_contra_claims)  # 7

        d3_c = sum(1 for t in gold_contra_claims if d3_map.get((t.question_id, t.arm_id, t.claim_id)) == "CONTRADICTED")
        d31_c = sum(1 for t in gold_contra_claims if d31_map.get((t.question_id, t.arm_id, t.claim_id)) == "CONTRADICTED")
        d32_c = sum(1 for t in gold_contra_claims if d32_map.get((t.question_id, t.arm_id, t.claim_id)) == "CONTRADICTED")

        return {
            "total_gold_contradicted_claims": total_gold,
            "d3_correctly_predicted_contradicted_count": d3_c,
            "d31_correctly_predicted_contradicted_count": d31_c,
            "d32_correctly_predicted_contradicted_count": d32_c,
            "d32_contradicted_recall": round(d32_c / total_gold if total_gold else 0.0, 4),
            "contradiction_discrimination_improved_vs_d3": bool(d32_c > d3_c),
        }

    def _compute_strict_conflict_diagnostics(
        self,
        targets: list[BenchmarkClaimTarget],
        d32_preds: list[dict[str, Any]],
    ) -> dict[str, Any]:
        ...
        strict_conflict_positives = 0
        true_contradictions = 0
        false_overrides_supported = 0
        false_overrides_insufficient = 0
        ...
        for t in targets:
            k = (t.question_id, t.arm_id, t.claim_id)
            p = pred_map.get(k, {})
            sa = p.get("structured_assessment") or {}
            conflict_ass = sa.get("conflict_assessment") or {}

            status = conflict_ass.get("status")
            same_prop = conflict_ass.get("same_material_proposition", False)
            cannot_both = conflict_ass.get("cannot_both_be_true", False)

            is_positive = (status == StrictConflictStatus.STRICT_CONTRADICTION_CONFIRMED.value) or (same_prop and cannot_both)

            if is_positive:
                strict_conflict_positives += 1
                if t.human_label == HumanEntailment.CONTRADICTED:
                    true_contradictions += 1
                elif t.human_label == HumanEntailment.SUPPORTED:
                    false_overrides_supported += 1
                elif t.human_label == HumanEntailment.INSUFFICIENT:
                    false_overrides_insufficient += 1

                details.append({
                    "claim_key": f"{t.question_id}:{t.arm_id}:{t.claim_id}",
                    "human_label": t.human_label.value,
                    "base_d3_label": p.get("base_d3_label"),
                    "final_d32_label": p.get("v2_d32_three_way_prediction"),
                    "override_applied": p.get("override_applied"),
                })

        false_positives = strict_conflict_positives - true_contradictions
        precision = (
            true_contradictions / strict_conflict_positives
            if strict_conflict_positives > 0
            else 0.0
        )
        recall = (
            true_contradictions / 7.0
        )

        return {
            "strict_conflict_positives_count": strict_conflict_positives,
            "true_human_contradicted_among_positives": true_contradictions,
            "false_strict_conflict_positives": false_positives,
            "strict_conflict_precision": round(precision, 4),
            "strict_conflict_recall": round(recall, 4),
            "false_overrides_by_class": {
                "SUPPORTED_to_CONTRADICTED": false_overrides_supported,
                "INSUFFICIENT_to_CONTRADICTED": false_overrides_insufficient,
            },
            "positive_conflict_claims_detail": details,
        }

    def _compute_d31_learning_diagnostic(
        self,
        targets: list[BenchmarkClaimTarget],
        d31_preds: list[dict[str, Any]],
        d32_preds: list[dict[str, Any]],
        ...
        for t in targets:
            k = (t.question_id, t.arm_id, t.claim_id)
            p31 = d31_map.get(k, {})
            # D3.1 contradiction positive
            if p31.get("v2_d31_three_way_prediction") == "CONTRADICTED" or p31.get("is_contradicted") is True:
                d31_positives_keys.append(k)

        retained_by_d32 = []
        filtered_by_d32 = []

        for k in d31_positives_keys:
            t = next(x for x in targets if (x.question_id, x.arm_id, x.claim_id) == k)
            p32 = d32_map.get(k, {})
            sa = p32.get("structured_assessment") or {}
            conflict_ass = sa.get("conflict_assessment") or {}
            is_pos_d32 = (
                conflict_ass.get("status") == StrictConflictStatus.STRICT_CONTRADICTION_CONFIRMED.value
                or (conflict_ass.get("same_material_proposition") and conflict_ass.get("cannot_both_be_true"))
            )
            item = {
                "claim_key": f"{k[0]}:{k[1]}:{k[2]}",
                "human_label": t.human_label.value,
                "d32_conflict_status": conflict_ass.get("status", "NO_STRICT_CONTRADICTION"),
                "d32_final_label": p32.get("v2_d32_three_way_prediction"),
            }
            if is_pos_d32:
                retained_by_d32.append(item)
            else:
                filtered_by_d32.append(item)

        return {
            "d31_contradiction_positives_total": len(d31_positives_keys),
            "retained_by_d32_strict_conflict_count": len(retained_by_d32),
            "filtered_out_by_d32_count": len(filtered_by_d32),
            "retained_human_label_distribution": dict(Counter(x["human_label"] for x in retained_by_d32)),
            "filtered_human_label_distribution": dict(Counter(x["human_label"] for x in filtered_by_d32)),
            "retained_claims": retained_by_d32,
            "filtered_claims": filtered_by_d32,
        }
    ...
    def _compute_gain_preservation_diagnostic(
        ...
        for t in claim_targets:
            ...
            if k in D3_FIX_CLAIM_KEYS:
                d32_val = d32_map.get(k)
                gold_val = t.human_label.value
                is_preserved = (d32_val == gold_val)
                if is_preserved:
                    preserved_count += 1
                else:
        ...
        for group_name, keys in FORENSIC_ERROR_GROUPS.items():
            ...
            for k in keys:
                ...
                items.append({
                    ...
                    "human_label": gold,
                    "d32_prediction": pred,
                    "is_fixed_in_d32": is_fixed,
                })
            groups_summary[group_name] = {
                "total_claims": total,
                "resolved_in_d32_count": resolved,
                "claims": items,
            }
    ...
    def _build_preflight_report(
        ...
        return {
            "schema_version": "1.0",
            "artifact_type": "v2_d32_development_benchmark_report",
            "candidate_id": self._candidate_id,
            "verdict": "V2_D32_DEVELOPMENT_BENCHMARK_READY",
            "sources_info": sources_info,
            "total_claims": len(claim_targets),
            ...
            "d31_diagnostic_metrics": {
                "d31_claim_binary": d31_binary,
                "d31_three_way": d31_three_way,
                "d31_answer_level": d31_answer,
            },
    ...
    def _build_reports(
        ...
        strict_conflict_diag: dict[str, Any],
        d31_learning_diag: dict[str, Any],
        ...
        total_structured_retries = (
            pass1_telemetry["total_structured_retries"] + pass2_telemetry["total_structured_retries"]
        )
        ...
        if base_drift:
            ...
            verdict = "V2_D32_DEVELOPMENT_EXECUTION_ERROR"
        elif stability_info["unstable_semantic_claim_count"] > 0:
            verdict = "V2_D32_DEVELOPMENT_LABEL_INSTABILITY"
        else:
            verdict = "V2_D32_DEVELOPMENT_BENCHMARK_PASS"

        # Pre-Registered Selection Gate: D32_SUPERSEDES_D3 vs KEEP_D3
        d32_correct_binary = d32_claim_binary["tp"] + d32_claim_binary["tn"]
        d32_supp_retained = d32_claim_binary["tp"]
        d32_neg_caught = d32_claim_binary["tn"]
        d32_correct_three_way = sum(
            d32_three_way["confusion_matrix"][c][c]
        ...
        mechanical_pass = (
            ...
            and d32_claim_binary["execution_errors"] == 0
            and d32_three_way["execution_errors"] == 0
        ...
        d32_supersedes_d3 = mechanical_pass and quality_pass
        dev_decision = "D32_SUPERSEDES_D3" if d32_supersedes_d3 else "KEEP_D3"
        ...
        final_report = {
            ...
            "metrics": all_metrics,
            "strict_conflict_diagnostics": strict_conflict_diag,
            "d31_learning_diagnostic": d31_learning_diag,
        ...
        decision_report = {
            "schema_version": "1.0",
            "artifact_type": "v2_d32_development_decision_report",
            "candidate_id": self._candidate_id,
            "verdict": verdict,
            "development_evaluation_decision": dev_decision,
            "d32_supersedes_d3": d32_supersedes_d3,
            ...
            "metrics_summary": {
                "d32_binary_correct": f"{d32_correct_binary}/38 (D3: 28/38, D3.1: 30/38)",
                "d32_supported_retained": f"{d32_supp_retained}/18 (D3: 17/18, D3.1: 12/18)",
    ...
    def _write_reports(
        ...
        strict_conflict_diag: dict[str, Any] | None = None,
        d31_learning_diag: dict[str, Any] | None = None,
        ...
        d31_claim_preds: list[dict[str, Any]] | None = None,
        ...
        if is_preflight:
            (results_dir / "v2_d32_development_report.json").write_text(
                json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8"
            )
            return

        if exec_identity:
            (exec_dir / "v2_d32_development_source_identity.json").write_text(
        ...
        if decision_report:
            (results_dir / "v2_d32_development_decision_report.json").write_text(
                json.dumps(decision_report, indent=2, ensure_ascii=False), encoding="utf-8"
            )

        if strict_conflict_diag:
            (results_dir / "v2_d32_strict_conflict_diagnostics.json").write_text(
                json.dumps(strict_conflict_diag, indent=2, ensure_ascii=False), encoding="utf-8"
            )

        if d31_learning_diag:
            (results_dir / "v2_d32_d31_learning_diagnostic.json").write_text(
                json.dumps(d31_learning_diag, indent=2, ensure_ascii=False), encoding="utf-8"
            )
        ...
        if d31_claim_preds:
            with (results_dir / "v2_d31_claim_predictions.jsonl").open("w", encoding="utf-8") as f:
                for r in d31_claim_preds:
                    f.write(json.dumps(r, ensure_ascii=False) + "\n")

        if pass1_claim_preds:
            with (results_dir / "v2_d32_claim_predictions_pass1.jsonl").open("w", encoding="utf-8") as f:
                for r in pass1_claim_preds:
                    f.write(json.dumps(r, ensure_ascii=False) + "\n")

        if pass2_claim_preds:
            with (results_dir / "v2_d32_claim_predictions_pass2.jsonl").open("w", encoding="utf-8") as f:
        ...
        if pass1_claim_preds and pass2_claim_preds:
            ...
            for p1 in pass1_claim_preds:
                ...
                comparisons.append({
                    ...
                    "pass1_three_way": p1["v2_d32_three_way_prediction"],
                    "pass2_three_way": p2.get("v2_d32_three_way_prediction"),
                    "pass1_base_d3": p1.get("base_d3_label"),
                    "pass2_base_d3": p2.get("base_d3_label"),
                    "pass1_override_applied": p1.get("override_applied"),
                    "pass2_override_applied": p2.get("override_applied"),
                    "stable": p1["v2_d32_three_way_prediction"] == p2.get("v2_d32_three_way_prediction"),
                })
            with (results_dir / "v2_d32_claim_comparisons.jsonl").open("w", encoding="utf-8") as f:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="V2-D3.2 Frozen D3 Base + Strict Contradiction Confirmation Benchmark Harness"
    )
    parser.add_argument(
        "--forensic-packets",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to verification-v2-d31-development-evidence.zip",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    evaluator = V2D32DevelopmentBenchmarkEvaluator(
        ...
        d31_evidence_path=args.d31_evidence,

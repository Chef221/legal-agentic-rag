...
def verify_b1a_configs(
    ...
    semantic_diffs = [
        d for d in diffs
        if not (d[0].startswith("artifacts.root_path") or d[0].endswith(".device"))
    ]

    if len(semantic_diffs) != 1 or semantic_diffs[0][0] != expected_diff_path:
        raise DataValidationError(
            f"Unexpected config semantic differences: {semantic_diffs}. "
            f"Expected ONLY {expected_diff_path}: True -> False"
        )

    path, val_base, val_cand = semantic_diffs[0]
    ...
    # Invariants checks
    assert base_app_config.online.retrieval.candidate_k == 40
    assert cand_app_config.online.retrieval.candidate_k == 40
    assert base_app_config.online.retrieval.top_k == 8
    assert cand_app_config.online.retrieval.top_k == 8
...
def _extract_routing_info(record: dict[str, Any]) -> dict[str, Any]:
    ...
    invocations = meta.get("tool_invocations") or []

    retrieval_tools = [
        inv["tool_name"] for inv in invocations
        if inv.get("tool_name") in {
            "bm25_search", "dense_search", "hybrid_search", "rerank_search", "graph_search"
        }
    ]
    graph_attempts = sum(1 for inv in invocations if inv.get("tool_name") == "graph_search")
    rerank_attempts = sum(1 for inv in invocations if inv.get("tool_name") == "rerank_search")
    hybrid_attempts = sum(1 for inv in invocations if inv.get("tool_name") == "hybrid_search")

    first_tool = retrieval_tools[0] if retrieval_tools else None
    final_strategy = meta.get("retrieval_strategy")
    ...
    return {
        ...
        "latency_ms": meta.get("agent_latency_ms", 0.0),
    }
...
def analyze_b1a_ablation(
    ...
    # Verify ID alignment
    if list(base_by_id.keys()) != ordered_qids:
        raise DataValidationError("BASE batch question IDs do not match canonical order")
    if list(cand_by_id.keys()) != ordered_qids:
        raise DataValidationError("CANDIDATE batch question IDs do not match canonical order")

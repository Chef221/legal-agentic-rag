#!/usr/bin/env python3
"""Positive-control candidate discovery and deterministic selection (B-FORENSIC-1B).

This script deterministically selects and materializes pre-registered positive-control
candidates from frozen Phase-A historical answer_verified outputs:
- 16 PRIMARY control candidates (4 per stratum across 4 strata)
- 8 RESERVE control candidates (2 per stratum across 4 strata)

Invariants:
- Selection algorithm does NOT use semantic verifiers, LLM judgment, or reference answers
- Selected candidates are 'positive-control candidates', NOT confirmed positives
- Human review status is strictly 'unreviewed' with null labels
...
# Excluded from control pool (22 relationship-case IDs including the 4 forensic target IDs)
...
STRATA_ORDER = [
    ControlStratum.D_NEGATION_MODALITY,
    ControlStratum.C_NUMERIC,
    ControlStratum.B_MULTI_CLAIM_CLEAN,
    ControlStratum.A_SINGLE_CLAIM_CLEAN,
]

STRATA_QUOTAS_PRIMARY = {
    ControlStratum.A_SINGLE_CLAIM_CLEAN: 4,
    ControlStratum.B_MULTI_CLAIM_CLEAN: 4,
    ControlStratum.C_NUMERIC: 4,
    ControlStratum.D_NEGATION_MODALITY: 4,
}

STRATA_QUOTAS_RESERVE = {
    ControlStratum.A_SINGLE_CLAIM_CLEAN: 2,
    ControlStratum.B_MULTI_CLAIM_CLEAN: 2,
    ControlStratum.C_NUMERIC: 2,
    ControlStratum.D_NEGATION_MODALITY: 2,
}
...
class SelectedControlCandidate:
    question_id: str
    stratum: str
    selection_key: str
...
class PositiveControlSelector:
    """Validate Phase-A source, perform deterministic stratified selection, and materialize packets."""
    ...
    def __init__(
        ...
        *,
        ...
        self._excluded_qids = excluded_qids or CANONICAL_EXCLUDED_RELATIONSHIP_IDS

    def run(self) -> dict[str, Any]:
        """Execute positive-control discovery, selection, and primary packet materialization."""
        ...
        try:
            ...
            if not (
                ...
                verdict = "INVALID_POSITIVE_CONTROL_PROVENANCE"
            else:
                verdict = "POSITIVE_CONTROL_SOURCE_READY"

            # 7. Build Standardized Reports
    ...
    def _resolve_phase_a_evidence(
        ...
        if path.is_file() and path.suffix.lower() == ".zip":
            ...
            temp_unpack = Path(tempfile.mkdtemp(prefix="phase_a_evidence_unpack_"))
    ...
    def _filter_eligible_pool(
        ...
        """Filter records to eligible answer_verified pool excluding relationship-case IDs."""
        eligible: list[dict[str, Any]] = []

        for qid, r in records_map.items():
            # Exclude relationship-case IDs
            if qid in self._excluded_qids:
                continue
    ...
    def _stratify_records(
    ...
    ) -> dict[ControlStratum, list[dict[str, Any]]]:
        """Classify records into exactly one of 4 deterministic strata using fixed precedence."""
        stratum_assignments: dict[ControlStratum, list[dict[str, Any]]] = defaultdict(list)
        ...
        for r in eligible_records:
            ...
            # Precedence: D -> C -> B -> A
            if is_negation:
                stratum_assignments[ControlStratum.D_NEGATION_MODALITY].append(r)
            elif is_numeric:
                stratum_assignments[ControlStratum.C_NUMERIC].append(r)
            elif claim_count >= 2 and is_clean:
                stratum_assignments[ControlStratum.B_MULTI_CLAIM_CLEAN].append(r)
            elif claim_count == 1 and is_clean:
                stratum_assignments[ControlStratum.A_SINGLE_CLAIM_CLEAN].append(r)
            else:
    ...
    def _sample_strata(
        self, stratum_assignments: dict[ControlStratum, list[dict[str, Any]]]
    ) -> tuple[list[SelectedControlCandidate], list[SelectedControlCandidate]]:
        """Deterministic sampling within each stratum using SHA-256 selection key."""
        primary_candidates: list[SelectedControlCandidate] = []
        reserve_candidates: list[SelectedControlCandidate] = []
        ...
        for stratum in STRATA_ORDER:
            ...
            for r in records:
                qid = r["question_id"]
                key = sha256_text(f"verification-positive-control-v1|{qid}")
    ...
    def _process_primary_case(
        self,
        *,
        candidate: SelectedControlCandidate,
        record: dict[str, Any],
        question_text: str,
        ...
        """Reconstruct evidence and replay verifier for one PRIMARY positive-control candidate."""
        qid = candidate.question_id
        ...
        try:
            ...
            _LOGGER.error("Replay exception on positive control %s: %s", qid, exc)
            replay_pass = False
            replay_dict = {
                "replay_applicable": True,
    ...
    def _build_report(
        ...
        *,
        ...
        chunk_manifest: Any,
        eligible_count: int,
        stratum_assignments: dict[ControlStratum, list[dict[str, Any]]],
        primary_candidates: list[SelectedControlCandidate],
        reserve_candidates: list[SelectedControlCandidate],
        primary_summaries: list[dict[str, Any]],
    ) -> dict[str, Any]:
        """Build standardized positive-control selection report."""
        return {
            "schema_version": "1.0",
            "verdict": verdict,
            "source_identity": {
            ...
            "candidate_sampling": {
                "sampling_algorithm": "deterministic_sha256_stratified_v1",
                "salt_prefix": "verification-positive-control-v1|",
                "excluded_relationship_id_count": len(self._excluded_qids),
    ...
    def _write_outputs(
        ...
        *,
        ...
        packets_dir = self._output_dir / "positive_control_packets"
        ...
        (exec_dir / "control_source_identity.json").write_text(
            json.dumps(identity, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )

        (results_dir / "control_selection_report.json").write_text(
            json.dumps(report, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )

        (results_dir / "primary_control_ids.json").write_text(
            json.dumps([asdict(c) for c in primary_candidates], ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )

        (results_dir / "reserve_control_ids.json").write_text(
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Select and materialize positive-control candidates from frozen Phase-A evidence."
    )
    parser.add_argument(
        "--phase-a-evidence",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Output directory for positive control selection artifacts.",
    )
    parser.add_argument(
        "--package-zip",
        type=Path,
        default=None,
        help="Optional output ZIP path for packaging review packets.",
    )
    ...
    if args.package_zip:
        ...
        print(f"\nPackaged review ZIP written to: {args.package_zip}")

...
2. **Typed Candidate Pool Narrowing:**
   - Added `relationship_candidate_k: int | None = Field(default=None, gt=0, le=100)` to `RerankerConfig`.
   - For `QueryIntent.RELATIONSHIP` queries, when `relationship_candidate_k` is configured, `RerankingRetriever` requests `min(query.candidate_k, relationship_candidate_k)` candidates (20 in M49.1) from the underlying candidate retriever, while preserving final `top_k` (10) and unmutated `query.candidate_k` (40).
   - Non-relationship queries and configs with `relationship_candidate_k = None` retain their normal candidate pool (40).

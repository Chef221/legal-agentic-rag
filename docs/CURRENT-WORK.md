...
5. **Active Research Frontier — Priority B (Verification-Correctness Audit)**:
   ...
   - **Next Action in Priority B**: Controlled V0 `RuleBasedCitationVerifier` vs V1 Semantic-Verifier Evaluation Design & Offline Execution (evaluating verifiers on the frozen 38-claim benchmark).
...
Current repository state:
    0.50.7 (Task B-FORENSIC-1C Complete — POSITIVE_CONTROL_HUMAN_LABELS_FROZEN; H40 Unpromoted; Model-backed Verifier Disabled)

Active development frontier:
    Priority B — Verification-correctness audit

Next action:
    Controlled V0 Rule-Based vs V1 Semantic-Verifier Evaluation Design
```
